...
<div class="container mt-5">
    <form action="{{ route('tambah-Admin') }}" method="post">
        ...
        <div class="mb-6">
            ...
            is-invalid
            @enderror" placeholder="nazalgusti" name="username" value="{{ old('username') }}" autocomplete="off">
            @error('username')
                <div class="feedback">
                    {{ $message }}
                </div>
        ...
        <input type="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('email')
            is-invalid
        @enderror" placeholder="admin@gmail.com" name="email" value="{{ old('email') }}" autocomplete="off">
        @error('email')
            <div class="feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-6">
            ...
            <input type="text" id="number_phone" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('no_hp')
                is-invalid
            @enderror" placeholder="0891283912" name="no_hp" value="{{ old('no_hp') }}" autocomplete="off">
            @error('no_hp')
                <div class="feedback">
                    {{ $message }}
                </div>
